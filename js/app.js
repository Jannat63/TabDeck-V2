...
function updateClock() {
  ...
  const colon = `<span class="clock-colon">:</span>`;
  let str = `${String(h).padStart(2,'0')}${colon}${String(m).padStart(2,'0')}`;
  if (S.settings.showSeconds) str += `${colon}${String(s).padStart(2,'0')}`;
  el('clockDisplay').innerHTML = str;
...
async function autoDetectWeather() {
  ...
  el('weatherWidget')?.classList.add('weather-loading');
...
function toggleTask(id) {
  ...
  if (!t) return;
  // Brief animation before re-render
  const row = document.querySelector(`.task-checkbox[data-tid="${taskId}"]`)?.closest('.task-item');
  if (row && !t.done) {
    row.style.transition = 'opacity .15s ease, transform .15s ease';
    row.style.transform = 'scale(1.01)';
    setTimeout(() => { row.style.transform = ''; }, 150);
  }
  t.done = !t.done; save(); renderTasksWidget();
}
...
function navigateTo(view) {
  ...
  if (current && current !== next) {
    current.style.animation = 'wsContentOut .12s ease forwards';
    setTimeout(() => {
      current.style.animation = '';
      document.querySelectorAll('.view').forEach(v => v.classList.remove('active'));
      if (next) {
        next.classList.add('active');
        next.style.animation = 'wsContentIn .2s cubic-bezier(.4,0,.2,1) both';
        setTimeout(() => { next.style.animation = ''; }, 220);
      }
...
function showToast(msg, type='') {
  ...
  t.innerHTML = (icon ? `<span class="toast-icon">${icon}</span>` : '') + `<span>${escH(msg)}</span>`;
